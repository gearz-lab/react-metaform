...
The most important metadata is a `field`. `Fields` exist in `entities`, `layout` and `groups`. When a field exist in both
an `entity` and a `layout`, they are matched by `name` and every other metadata is merged, and the `layout` always wins.
